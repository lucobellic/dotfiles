...
async fn main() -> anyhow::Result<()> {
  ...
  let Some(active_window) = active_window else {
    send_notification("Workspace Association", "No active window found", "low")?;
    anyhow::bail!("No active window found");
  };

  let window_class = &active_window.class;
  // let window_title = &active_window.initial_title;
  ...
  let rule = format!("workspace {}, match:class {}", workspace_id, escaped_class);
  Keyword::set("windowrule", rule.clone()).context("Failed to set window rule")?;

  send_notification("Workspace Association", &rule, "low")?;
  println!("{}", &rule);
